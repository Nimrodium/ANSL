...
// pub fn preprocessor_engine(stream: TokenStream) -> Result<TokenStream, CompilerError> {
//     let mut inside_instruction = false;
//     let mut instruction: Option<&str> = None;
//     let mut arguments: Vec<&Token> = Vec::new();
//     let mut instruction_ready = false;
//     let mut instruction_start = 0;
//     let mut instruction_end = 0;
//     let mut instruction_token_length = 0;
//     let mut include_files: Vec<(TokenStream, (usize, usize))> = Vec::new();

//     for (i, token) in &mut stream.tokens.iter().enumerate() {
//         if token.kind == TokenKind::PreProcessorInstruction {
//             inside_instruction = true;
//             instruction = Some(stream.dereference_string(token.lexeme)?);
//             continue;
//         }
//         if inside_instruction {
//             instruction_token_length += 1;
//             match token.kind {
//                 TokenKind::PreProcessorArgument => {
...
//                 _ => {
//                     return Err(CompilerError::new(&format!(
//                         "unexpected token [{:?} \"{}\" ] in preprocessor instruction",
//                         token.kind,
//                         stream.dereference_string_fallback(token.lexeme)
//                     )))
//                 }
//             }
//         }
...
//             let inst = if let Some(i) = instruction {
//                 i
//             } else {
//                 return Err(CompilerError::new("bad state instruction missing"));
//             };

//             match inst {
//                 "#include" => {
//                     let location = if let Some(s) = arguments.get(0) {
//                         stream.dereference_string(s.lexeme)?
//                     } else {
//                         return Err(CompilerError::new(
//                             "missing location in preprocessor instruction",
//                         ));
//                     };
//                     let name = if let Some(s) = arguments.get(1) {
//                         stream.dereference_string(s.lexeme)?
//                     } else {
//                         return Err(CompilerError::new(
//                             "missing name in preprocessor instruction",
//                         ));
//                     };
//                     let included_file_tokenstream = match location {
//                         "system" => {
//                             let path = (SYSTEM_LIB.to_string() + name) + ".ansl";
//                             tokenize_file(&path)?
//                         }
//                         "module" => {
//                             let path = ("./".to_string() + name) + ".ansl";
//                             tokenize_file(&path)?
//                         }
//                         "absoulute" => tokenize_file(&name)?,
//                         _ => {
//                             return Err(CompilerError::new(&format!(
//                                 "invalid include argument {}",
//                                 location
...
//                     };
//                     include_files.push((
//                         included_file_tokenstream,
//                         (instruction_start, instruction_end),
//                     ));
//                 }
//                 _ => {
//                     return Err(CompilerError::new(&format!(
//                         "invalid preprocessor command {inst}"
//                     )))
//                 }
//             }
//         }
//     }
//     let mut new_token_stream = TokenStream::new(stream.str_db);
//     for token in &stream.tokens {
//         if !(token.kind == TokenKind::PreProcessorInstruction
//             || token.kind == TokenKind::PreProcessorArgument)
//         {
//             new_token_stream.tokens.push(token.clone());
//         }
//     }
//     Ok(())
// }
